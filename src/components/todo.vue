...
import { authData, todoData } from "../data/data";
const auth = authData();
const todo = todoData();
onBeforeMount(() => {
    todo.gettodoitems();
    ...
    <div class="flex items-center justify-center font-medium">
        <div class="flex flex-grow items-center justify-center h-full text-gray-600 bg-gray-100">
            ...
            <div class="max-w-full p-8 bg-white rounded-lg shadow-lg w-96">
                ...
                <div v-for="(task, index ) in   todo.todoItemList.todos  " :key="index">

                    <input @change="todo.updateItems(task)" class="hidden" type="checkbox" :id="'task_' + index"
                    ...
                    <label class="flex items-center h-10 px-2 rounded cursor-pointer hover:bg-gray-100"
                        ...
                        <span class="ml-4 text-sm">{{ task.todo }}</span>
                    </label>
                </div>
                <div>
